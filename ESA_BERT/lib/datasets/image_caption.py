...
class RawImageDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.train:
            target = []
            target_len = []
            for i in range (self.opt.hardnum):
                target_i = process_caption(self.tokenizer, caption_tokens, self.train)
            ...
            return image,target,target_len,index,img_index
        else:
...
class PrecompRegionDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.train:
            ...
            target_len = [] 
            for i in range(self.opt.hardnum):
                target_i = process_caption(self.tokenizer, caption_tokens, self.train, 0.2)
                target_len.append(len(target_i))
                target.append(target_i)
            images = self.images[img_index]
            # img_len = len(images)
            images = torch.Tensor(images)
            return images, target, target_len, index, img_index
        else:
            target = process_caption(self.tokenizer, caption_tokens, self.train)
            images = self.images[img_index]
            images = torch.Tensor(images)
...
def collate_fn(data):
    ...
    Returns:
        ...
        lengths: list; valid length for each padded caption.
    """
    images, captions, cap_lens, ids, img_ids = zip(*data)
    if len(images[0].shape) == 2:  # region feature
        # Merge images
        img_lengths = [len(image) for image in images]
        all_images = torch.zeros(len(images), max(img_lengths), images[0].size(-1))
        ...
        # Merget captions
        cap_len = torch.Tensor(cap_lens).long()
        all_captions = torch.zeros(len(captions),len(cap_len[0]),cap_len.max()).long()#b,hardnum.maxlen
        for i,cap in enumerate(captions):
            for j,index in enumerate(cap_lens[i]):
                end = index
                all_captions[i,j,:end] = cap[j][:end]
        cap_lens = torch.Tensor(cap_lens)
        return images, all_captions, cap_lens, ids
